"""
Basic Consumer Example for redisaq (Consumer 2)
...
Behavior:
- Registers with group 'email_group', consumer_id 'consumer_2'.
...
async def process_job(job):
    logger.info(f"Consumer 2 processing job {job.id} with payload {job.payload}")
    await asyncio.sleep(random.uniform(0.5, 1.5))  # Simulate processing
    logger.info(f"Consumer 2 completed job {job.id}")
