...
async def test_connect(consumer, redis_client, mock_aioredis_from_url):
    ...
    mock_aioredis_from_url.assert_called_once_with(consumer.redis_url, decode_responses=True)
...
async def test_register_consumer(consumer, mock_aioredis_from_url):
    ...
    assert consumer.deserializer.loads(value) is False  # _is_ready defaults to False
...
async def test_get_consumers(consumer, mock_aioredis_from_url):
    ...
    await consumer.connect()
    consumers_key1 = f"{consumer._topic_keys.consumer_group_keys.consumer_key}:consumer1"
    consumers_key2 = f"{consumer._topic_keys.consumer_group_keys.consumer_key}:consumer2"
    await consumer.redis.set(consumers_key1, consumer.serializer.dumps(True))
    await consumer.redis.set(consumers_key2, consumer.serializer.dumps(False))
...
async def test_signal_rebalance(consumer, mock_aioredis_from_url):
    ...
    for _ in range(2):
        ...
        if not (message and message["type"] == "message" and message["data"] == "rebalance"):
            continue
...
async def test_remove_ready(consumer, mock_aioredis_from_url):
    ...
    consumers_key = f"{consumer._topic_keys.consumer_group_keys.consumer_key}:{consumer.consumer_name}"
    value = await consumer.redis.get(consumers_key)
    assert consumer.deserializer.loads(value) is False
...
async def test_all_consumers_ready(consumer, mock_aioredis_from_url):
    ...
    consumers_key2 = f"{consumer._topic_keys.consumer_group_keys.consumer_key}:consumer2"
    await consumer.redis.set(consumers_key1, consumer.serializer.dumps(True))
    await consumer.redis.set(consumers_key2, consumer.serializer.dumps(True))
...
async def test_wait_for_all_ready(consumer, mock_aioredis_from_url):
    ...
    # Add consumer to Redis
    consumers_key = f"{consumer._topic_keys.consumer_group_keys.consumer_key}:{consumer.consumer_name}"
    await consumer.redis.set(consumers_key, consumer.serializer.dumps(False))
...
async def test_heartbeat(consumer, mock_aioredis_from_url):
    ...
    value = await consumer.redis.get(consumers_key)
    assert value is not None
    assert consumer.deserializer.loads(value) is False  # _is_ready remains False
